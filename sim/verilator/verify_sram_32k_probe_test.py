...
def must_fail(
    ...
    try:
        verify(type01_rom, type01_trace, type02_rom, type02_trace)
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="swansong-sram32k-test-") as directory:
        root = Path(directory)
        type01_rom, type02_rom = generate(root)
        ...
        verify(type01_rom, traces[1], type02_rom, traces[2])
        ...
        must_fail(type01_rom, traces[1], type02_rom, traces[2], "mapped_offset mismatch")
        ...
        for name, index, field, value, expected in mutations:
            ...
            must_fail(type01_rom, traces[1], type02_rom, traces[2], expected)

        write_trace(traces[1], valid_rows()[:-1], 1, roms[1])
        must_fail(type01_rom, traces[1], type02_rom, traces[2], "event count")
        ...
        must_fail(type01_rom, traces[1], type02_rom, traces[2], "cycle mismatch")

        write_trace(traces[2], valid_rows(), 2, roms[2])
        for name, updates, expected in (
            ("rom-binding", {"rom_fnv1a64": "0" * 16}, "rom_fnv1a64 mismatch"),
            ("open-ipl-binding", {"open_ipl_fnv1a64": "0" * 16}, "open_ipl_fnv1a64 mismatch"),
        ...
        write_trace(traces[1], valid_rows(), 1, roms[1])
        must_fail(corrupt, traces[1], type02_rom, traces[2], "size/hash")
