#!/usr/bin/env python3
"""Positive and adversarial tests for the internal-EEPROM target verifier."""
...
from verify_internal_eeprom_fixture import (
    BG_FIELDS,
    CAPTURE_CYCLES,
    CAPTURE_FRAMES,
    CPU_FIELDS,
    DEFAULT_MONO_OPEN_IPL_FNV1A64,
    EXPECTED_EVENTS,
    FONT_OFFSET,
    FONT_SIZE,
    FRAME_SIZE,
...
def must_fail(call, expected: str) -> None:
    try:
        ...
        raise AssertionError(
            f"invalid internal-EEPROM fixture passed; expected {expected!r}"
        )


def main() -> None:
    repository = Path(__file__).resolve().parents[2]
    checked_fixture = repository / "testroms/ws-test-suite/internal_eeprom"
    verify_sources(checked_fixture)
    checked_rom = verify_rom(checked_fixture / ROM_NAME)
    frame = expected_frame(checked_rom)
    assert len(RESULT_POSITIONS) == 23
    assert len(frame) == FRAME_SIZE
    assert sha256(frame) == PASS_FRAME_SHA256

    with tempfile.TemporaryDirectory(prefix="swansong-internal-eeprom-test-") as directory:
        root = Path(directory)
        ...
        assert counts["pass_results"] == 23
        assert counts["pass_rows"] == 184
        ...
        must_fail(lambda: verify_trace(wrong_position, rom), "unexpected (25, 1)")
        ...
        map_rows = deepcopy(rows)
        map_rows[first_bg]["map_address"] = int(map_rows[first_bg]["map_address"]) + 2
        write_trace(wrong_map, map_rows, rom)
        ...
        early_rows = deepcopy(rows[first_bg:])
        for index, row in enumerate(early_rows):
            row["cycle"] = 10 + index
        early_rows.append(cpu_row(1000, 0xFFFF2, 0xFFFF, 2))
        early_rows.extend(
            cpu_row(2000 + index, TERMINAL_PC, TERMINAL_CS, TERMINAL_IP)
            for index in range(MIN_TERMINAL_TAIL)
        )
        write_trace(early_only, early_rows, rom)
        must_fail(lambda: verify_trace(early_only, rom), "post-terminal PASS raster")

        bad_frame = root / "bad-frame.rgb"
        changed_frame = bytearray(frame)
        changed_frame[0] ^= 0xFF
        bad_frame.write_bytes(changed_frame)
        must_fail(lambda: verify_frame(bad_frame, rom), "23-PASS frame")
        ...
        manifest["complete_bg_cell_history"] = False
    ...
    print(
        "PASS pinned internal-EEPROM verifier "
        "source/footer/checksum/terminal/23-results/rows/frame/manifest/"
