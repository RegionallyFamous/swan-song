...
BUILD="$(mktemp -d "$ROOT/build/sim/gpu_sprite_dma_timing_tb.XXXXXX")"
...
docker run --rm --platform linux/amd64 \
  ...
  ghdl -a --std=08 -frelaxed-rules --workdir=. \
    ...
    /work/sim/rtl/gpu_sprite_dma_timing_tb.vhd

docker run --rm --platform linux/amd64 \
  -v "$ROOT:/work" -w "$BUILD_CONTAINER" "$IMAGE" \
  ghdl -e --std=08 -frelaxed-rules --workdir=. \
    -o gpu_sprite_dma_timing_tb gpu_sprite_dma_timing_tb

set +e
output="$(docker run --rm --platform linux/amd64 \
  -v "$ROOT:/work" -w "$BUILD_CONTAINER" "$IMAGE" \
  ./gpu_sprite_dma_timing_tb --assert-level=error 2>&1)"
status=$?
set -e
printf '%s\n' "$output"
if (( status != 0 )); then
...
grep -q 'gpu_sprite_dma_timing_tb.vhd:.*PASS GPU sprite DMA line-144 timing at all eight arbiter phases, wrap data, boundary latch, provenance, and reset cancellation' <<<"$output"
