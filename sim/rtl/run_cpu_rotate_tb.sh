...
BUILD="$(mktemp -d "$ROOT/build/sim/cpu_rotate_tb.XXXXXX")"
...
sources=(
  ...
  "$CPU_RTL_CONTAINER"
  /work/sim/rtl/cpu_rotate_tb.vhd
)
...
docker run "${docker_args[@]}" "$IMAGE" \
  ghdl -e --std=08 -frelaxed-rules --workdir=. \
    -o cpu_rotate_tb cpu_rotate_tb

docker run "${docker_args[@]}" "$IMAGE" \
  ./cpu_rotate_tb --assert-level=error
...
test -s "$BUILD/cpu.v"
echo "PASS GHDL synthesized CPU after rotate regression test"
