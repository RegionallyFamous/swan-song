...
`f0345ee4bae92cf137c600dfca876494cb17a5fe`; the exact public-release commit is
not frozen yet.
...
paths are source-verified below. The protected-main Quartus candidate passes its
accepted evidence audit, and a second clean build produces a byte-identical RBF
and build ID. Physical Pocket/Dock gates, distribution clearance, and two fresh
signed workflow executions after release-facing metadata or policy changes
remain open.
...
builder, accepted protected-main Quartus candidate, four-corner timing audit,
and byte-identical rebuild are strong software and FPGA evidence;
...
3. Exercise Interact persistence, the complete video/audio path, BIOS prompts,
   title changes, screenshots, and Dock transport on physical Pocket hardware;
...
5. After release metadata and policy are frozen, produce two clean Quartus
   21.1.1 candidates for that exact public commit and epoch in distinct signed
   workflow executions with different fresh job nonces. Both must pass the full
   candidate audit and produce byte-identical RBF and build-ID files. Feed both
   ...
   distinct workflow executions, not distinct physical hosts. The protected-main
   `f0345ee4` package remains a historical hardware-QA candidate, not a stable
   release that can be promoted without rebuilding.
...
| P1-I/V | Packaging | **Host release gate and accepted candidate evidence complete; final release V2 and authorization absent** | Packaging requires a nonempty RBF, applies a complete release-path allowlist, validates every shipped definition and artwork contract, materializes the bit-reversed stream and pinned Chip32 image, and emits a deterministic ZIP plus SHA-256 provenance sidecar ([`package_core.py`](scripts/package_core.py), [`package_validator.py`](scripts/package_validator.py)). Release mode requires the metadata-derived `Author.Core_version_date.zip` name and `SWAN_SONG_RELEASE_EVIDENCE_V2`. V2 binds the exact RBF, generated build-ID MIF, Quartus 21.1.1 reports, and two candidate audits from distinct signed workflow runs that the packager recomputes from the complete artifact bundles; the protected-main `f0345ee4` candidate and byte-identical rebuild satisfy the historical development-candidate build proof, but newer changes require fresh signed builds. The candidate audit deliberately retains `release_eligible: false` and cannot claim compression or hardware. Final V2 packaging re-verifies and hash-binds the exact private inventory and accepted 33-case Pocket/Dock manifest, then requires its RBF, installed bitstream, `core.json`, `interact.json`, version, date, and nine persistent settings to match the release; bare booleans cannot authorize it. Release-policy V2 records `agg23.WonderSwan` 1.0.0/1.0.1 as predecessor history, approves the independent `RegionallyFamous.SwanSong` identity and repository, leaves Swan Song's published list empty, and keeps distribution-and-licensing authorization false. The first Swan Song release has no agg23 version/date floor; strict Semantic Version and date monotonicity begin only after Swan Song has a published entry ([`release-policy.json`](release-policy.json), [`package_core_test.py`](scripts/package_core_test.py)). | Complete the physical 33-case run—including the exact Auto Off dirty-save case—and licensing review, choose the first public Swan Song version/date/firmware floor, authorize distribution-and-licensing, and freshly reproduce the exact release commit in two distinct signed workflow executions before running the stable assembler plan/apply path; no stable release package is currently claimed. |
| P0-I/V | Quartus bitstream and TimeQuest | **Protected-main candidate accepted and byte-identically reproduced; physical/public-release gates open** | The pinned Quartus Lite 21.1.1 Linux/amd64 flow completed fresh synthesis, fitting, assembly, RBF generation, and four-corner TimeQuest for `5CEBA4F23C8` from protected `main` commit `f0345ee4bae92cf137c600dfca876494cb17a5fe`, using the current 5.9/2.5 ns SDRAM constraints. It fits at 13,207/18,480 logic elements, 2,622,848/3,153,920 memory bits, and 289/308 RAM blocks. Minimum setup/hold/recovery/removal/pulse-width slack is `+0.426/+0.030/+3.679/+0.264/+0.753 ns`; all eight `check_timing` diagnostics and unconstrained-path counts are zero; all reviewed connectivity rows match the exact allowlist; and there are zero critical warnings. A separate clean DigitalOcean build produces the same `89cd72d…09ee5` RBF and `dd62385c…0b43` build ID byte-for-byte. The candidate audit passes but correctly records `release_eligible: false` because physical Pocket/Dock and release-only gates are absent. | Use this exact candidate for controlled Pocket/Dock QA. After licensing, policy, version, and release metadata are frozen, produce two candidates for the exact public commit and epoch in distinct signed workflow executions with different fresh job nonces, require byte-identical RBF/build-ID outputs, and pass both complete bundles to the stable assembler; do not promote the development package without rebuilding. |
