...
|---|---|---|---|---|
| P0-I | Framework level and APF baseline | **Partial** | The core declares APF v1 metadata and now requires framework `2.0`, the version that introduced display modes and `00B8`; Dock remains enabled while `sleep_supported` is deliberately false until Memories is certified ([`core.json`](dist/Cores/agg23.WonderSwan/core.json)). Analogue's current changelog says the framework is `2.3`; the public `core-template` is still a 2023 v1.3.0 reference that declares `1.1`, so it is not the current protocol authority. The PAD wrapper and top-level now retain the official 32-bit key word, accept Pocket/Dock gamepad types, and reject absent, keyboard, mouse, and reserved types with focused RTL and mutation coverage ([`io_pad_controller.v`](src/fpga/apf/io_pad_controller.v), [`apf_gamepad_filter.sv`](src/fpga/core/apf_gamepad_filter.sv)). | Treat `version_required` as the minimum actually needed, not a marketing version. Keep the typed-PAD and host-definition gates green; test on framework 2.0 and current 2.3 behavior, document the compatibility range, and resolve the remaining lifecycle/hardware gaps before release. |
...
| P1-V | Base video bus/timing | **Source safety and focused units verified; physical timing unverified** | The definition is 224x144 RGB with 0/270/180-degree scaler slots ([`video.json`](dist/Cores/agg23.WonderSwan/video.json)). Slot changes cross atomically and apply only at VS; the related 36.864/6.144 MHz RGB/control bundle is staged on the intervening system falling edge and remains visible to TimeQuest. The buffered path uses five-bank ownership so three immutable completed histories, one pending frame, and the writer cannot alias; faster producer frames supersede only pending work, title reset invalidates history, and exact two/three-frame blending preserves equal inputs and white ([`apf_scaler_selector.sv`](src/fpga/core/apf_scaler_selector.sv), [`apf_framebank_arbiter.sv`](src/fpga/core/apf_framebank_arbiter.sv), [`apf_temporal_blend.sv`](src/fpga/core/apf_temporal_blend.sv)). Focused benches and source contracts cover these invariants. There is not yet a complete bus-level timing assertion suite or Quartus report. | Add assertions for one VS/frame, one HS/line, HS-to-DE and DE-to-next-HS gaps, active 224x144, zero/reserved words outside DE except the scaler-slot command, stable RGB888, and 47-61 Hz. Gate on Pocket/Dock for all rotations, direct/buffered output, flicker modes, tearing/latency, display modes, and screenshots. |
