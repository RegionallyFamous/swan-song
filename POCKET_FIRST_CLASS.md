...
Adam Gastineau Pocket port, and spiritualized1997 platform-art credits and
historical links remain in `README.md` and `UPSTREAMS.md`.
...
ROMs/BIOS files and slot-0-mirrored cartridge saves remain shared; fixed console
EEPROM, Settings, Presets, browser history, and Memories are separate core-ID
namespaces. EEPROM/settings/presets may be copied only with a backup, no
destination overwrite, and post-copy validation. Memories must not be copied:
they are disabled and there is no cross-ID format migration.
...
5. Produce a clean Quartus 21.1.1 build, pass TimeQuest with no unconstrained or
...
| P1-V | `00B0` menu behavior | **Compliant (source), hardware pending** | `00B0` is accepted and stored ([`core_bridge_cmd.v:461-465`](src/fpga/core/core_bridge_cmd.v#L461-L465)); the signal is otherwise unused ([`core_top.v:573-649`](src/fpga/core/core_top.v#L573-L649)). Officially this notification may be ignored for compatibility. | Keep the explicit OK response. Pocket gate: repeated menu entry/exit must not stick buttons, alter fast-forward, disturb audio, corrupt a Memory, or break return-to-game. Pausing in the menu is a product choice, not a protocol requirement. |
...
| P1-I/V | Packaging | **Host release gate complete; licensing authorization and accepted build evidence absent** | Packaging requires a nonempty RBF, applies a complete release-path allowlist, validates every shipped definition and artwork contract, materializes the bit-reversed stream and pinned Chip32 image, and emits a deterministic ZIP plus SHA-256 provenance sidecar ([`package_core.py`](scripts/package_core.py), [`package_validator.py`](scripts/package_validator.py)). Release mode requires the metadata-derived `Author.Core_version_date.zip` name and a strict evidence manifest bound to the exact RBF, generated build-ID MIF, Quartus 21.1.1 reports, and accepted timing/warning/Pocket/Dock gates. Release-policy V2 separately records `agg23.WonderSwan` 1.0.0/1.0.1 as predecessor history, approves the `RegionallyFamous.SwanSong` identity and repository, leaves Swan Song's own published list empty, and keeps distribution-and-licensing authorization false. The first Swan Song release has no agg23 version/date floor; strict Semantic Version and date monotonicity begin only after Swan Song has a published entry. Twenty-two focused tests cover deterministic development packages, strict V2 schemas and authorization separation, predecessor isolation, current-history monotonicity, allowlists/assets/leaks, evidence tampering, failed gates, and successful evidence/policy binding ([`release-policy.json`](release-policy.json), [`package_core_test.py`](scripts/package_core_test.py)). | Complete the licensing review, produce and accept real Quartus/Pocket evidence, choose the first public Swan Song version and actual release date, authorize distribution-and-licensing, then exercise `--release`; no release package is currently claimed. |
