...
"""Validate the SWANFRAME ROM, emulator evidence, and audio proof."""
...
INTERFACE_MASTER = ROOT / "assets/source-art/swanframe-sd-mobile-suit-imagegen-v5.png"
INTERFACE_RUNTIME = ROOT / "assets/runtime/swanframe-sd-mascot-v5.png"
UI_CONCEPT = ROOT / "assets/source-art/swanframe-ui-concept-v2-night-drive.png"
...
UI_CONCEPT_SHA256 = (
    "50d3ecd97bec0f1509ff3d91dd7f5e0c77326648ba71f11631cadf937f92f813"
)
...
def inspect_interface_art() -> dict[str, object]:
    ...
    concept_size = png_size(UI_CONCEPT)
    ui_tiles_size = png_size(UI_TILES)
    master_hash = sha256(INTERFACE_MASTER)
    concept_hash = sha256(UI_CONCEPT)
    if master_hash != INTERFACE_MASTER_SHA256:
        raise ValueError("ImageGen interface master changed without provenance review")
    if concept_hash != UI_CONCEPT_SHA256:
        raise ValueError("ImageGen Night Drive UI concept changed without provenance review")
    ...
    if concept_size != (1564, 1006):
        raise ValueError(f"unexpected Night Drive concept dimensions: {concept_size}")
    if ui_tiles_size != (64, 8):
        raise ValueError(f"runtime UI sheet is not eight 8x8 tiles: {ui_tiles_size}")
    return {
        ...
        "uiConcept": {
            "path": str(UI_CONCEPT.relative_to(ROOT)),
            "sha256": concept_hash,
            "width": concept_size[0],
            "height": concept_size[1],
            "colorway": "Night Drive",
        },
        "runtimeUIGlyphs": {
            "path": str(UI_TILES.relative_to(ROOT)),
            "sha256": sha256(UI_TILES),
            "width": ui_tiles_size[0],
            "height": ui_tiles_size[1],
            "tileCount": ui_tiles_size[0] // 8,
        },
    }
...
def main() -> int:
    ...
    report = {
        "schema": "swanframe-release-verification-v1",
        "status": "pass",
        "release": "0.1.0",
        "rom": rom,
        "soundtrack": {
