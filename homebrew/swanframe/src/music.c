...
static const swanframe_instrument_t __far instruments[SWANFRAME_INSTRUMENT_COUNT] = {
...
static uint16_t note_to_frequency(uint8_t note) {
	if (note == SWANFRAME_NOTE_NONE || note == SWANFRAME_NOTE_OFF ||
...
static void release_voice(swanframe_voice_t *voice) {
	if (voice->env_stage != ENV_OFF) voice->env_stage = ENV_RELEASE;
}

static void trigger_voice(uint8_t channel, const swanframe_event_t __far *event) {
	swanframe_voice_t *voice = &voices[channel];
	const swanframe_instrument_t __far *instrument;

	if (event->note == SWANFRAME_NOTE_OFF) {
		release_voice(voice);
		return;
	}
	if (event->note == SWANFRAME_NOTE_NONE || event->instrument >= SWANFRAME_INSTRUMENT_COUNT) return;
...
static void process_row(void) {
	const swanframe_song_t __far *song = &swanframe_songs[current_song];
	uint8_t pattern_index = song->order[current_order];
	const swanframe_pattern_t __far *pattern = &swanframe_patterns[pattern_index];
	uint8_t channel;

	for (channel = 0; channel < SWANFRAME_CHANNEL_COUNT; channel++) {
...
static void advance_row(void) {
	const swanframe_song_t __far *song = &swanframe_songs[current_song];
	current_row++;
	if (current_row >= SWANFRAME_PATTERN_ROWS) {
		current_row = 0;
		current_order++;
...
static uint16_t voice_frequency(swanframe_voice_t *voice, const swanframe_instrument_t __far *instrument) {
...
static void update_voice(uint8_t channel) {
	swanframe_voice_t *voice = &voices[channel];
	const swanframe_instrument_t __far *instrument = &instruments[voice->instrument];
	...
	case ENV_RELEASE:
		if (voice->env_level <= instrument->release_rate) {
			...
			voice->note = SWANFRAME_NOTE_NONE;
		} else {
...
void swanframe_music_init(void) {
	...
	for (channel = 0; channel < SWANFRAME_CHANNEL_COUNT; channel++) {
		voices[channel].instrument = SWANFRAME_INST_LEAD;
		voices[channel].env_stage = ENV_OFF;
		set_channel_wave(channel, WAVE_SQUARE);
	}
	update_channel_control();
	swanframe_music_select(0);
}
...
void swanframe_music_update(void) {
	...
	for (channel = 0; channel < SWANFRAME_CHANNEL_COUNT; channel++) update_voice(channel);
...
void swanframe_music_select(uint8_t song_index) {
	uint8_t channel;
	if (song_index >= swanframe_song_count) song_index = 0;
...
void swanframe_music_restart(void) {
	swanframe_music_select(current_song);
}
...
void swanframe_music_set_speed_index(uint8_t new_speed_index) {
...
void swanframe_music_toggle_mute(uint8_t channel) {
	if (channel < SWANFRAME_CHANNEL_COUNT) mute_mask ^= (uint8_t)(1u << channel);
}

uint8_t swanframe_music_song_index(void) { return current_song; }
uint8_t swanframe_music_master_volume(void) { return master_volume; }
uint8_t swanframe_music_speed_index(void) { return speed_index; }
uint8_t swanframe_music_speed_percent(void) { return speed_percentages[speed_index]; }
bool swanframe_music_is_paused(void) { return paused; }
bool swanframe_music_is_muted(uint8_t channel) {
	return channel < SWANFRAME_CHANNEL_COUNT && (mute_mask & (1u << channel));
}
uint8_t swanframe_music_channel_note(uint8_t channel) {
	return channel < SWANFRAME_CHANNEL_COUNT ? voices[channel].note : SWANFRAME_NOTE_NONE;
}
uint8_t swanframe_music_channel_level(uint8_t channel) {
	return channel < SWANFRAME_CHANNEL_COUNT ? voices[channel].meter : 0;
}
uint8_t swanframe_music_channel_instrument(uint8_t channel) {
	return channel < SWANFRAME_CHANNEL_COUNT ? voices[channel].instrument : 0;
}
uint16_t swanframe_music_step(void) {
	return (uint16_t)current_order * SWANFRAME_PATTERN_ROWS + current_row;
}
uint16_t swanframe_music_total_steps(void) {
	return (uint16_t)swanframe_songs[current_song].order_length * SWANFRAME_PATTERN_ROWS;
}
const swanframe_song_t __far *swanframe_music_song(void) { return &swanframe_songs[current_song]; }
const char __far *swanframe_music_instrument_name(uint8_t instrument) {
	return instruments[instrument < SWANFRAME_INSTRUMENT_COUNT ? instrument : 0].name;
}
