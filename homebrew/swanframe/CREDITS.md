# Credits and provenance

- **SWANFRAME** title, original SD mobile-suit technician direction, interface, native
  cartridge player, and five-song score: newly created for this project.
- OpenAI ImageGen: original SWANFRAME visual-development master, generated from
  the archived prompt in `assets/source-art/`; the runtime derivative and exact
  interface typography were selected and built specifically for this ROM.
- **Humming Cat** compact sound-driver/data-player concept: DOI Hiroyuki, beta
  04, May 12, 2002. It is credited as a technical research reference, not as
  SWANFRAME's name, mascot, artwork, soundtrack, or distributed code.
- Wonderful Toolchain and WonderSwan system libraries: Adrian “asie” Siekierka
  and contributors; linked system libraries are zlib-licensed and the GCC
  runtime uses the GCC Runtime Library Exception. See `THIRD_PARTY_NOTICES.md`.
- Swan Song desktop playtest engine: its bundled pinned ares backend and Swan
  Song contributors, with exact build identity recorded in release evidence.
- Mednafen: independent WonderSwan boot, checksum, and WAV-recording proof.

Humming Cat research reference:
<http://wwgp.qute.co.jp/2002/entry/00073/hcbeta04.lzh>

No Humming Cat binary, compiler, sample song data, name treatment, or mascot art
is included in SWANFRAME.

SWANFRAME's project-authored material is released under GPL-3.0-or-later. See
`COPYING` and `release/LICENSE_STATUS.md`.
