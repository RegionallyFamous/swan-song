...
| Memories SDRAM ownership boundary | Live ROM pass-through plus disabled stage side and global drain focused-simulated; Memories/Sleep still unsupported | channel 1 is runtime-idle after cartridge setup and now passes through an exclusive owner that latches requests until completion, gives held cartridge work acquisition priority, changes owner only after drain, routes ready/data to the latched owner, preserves a held ROM request across release, and fails closed on illegal staging access; the SDRAM controller now clears all queued/delayed transaction state on PLL init and exports global quiescence across request, command, CAS, ready, cooldown, refresh, and startup phases, while the wrapper retains named channel-2/channel-3 requests plus channel-3 ready; staging acquisition/request inputs remain tied low until cooperative instruction-boundary pause, real channel-3 state-engine acknowledgement, serialized transfer, lossless CDC, and exact payload padding exist; `savestate_supported=0` and `sleep_supported=false` are unchanged |
| Memories v2 RTC/EEPROM device boundaries | Exact local ABI plus isolated owner/backing walker verified; production integration open | the RTC exports a fixed 32-byte semantic-big-endian image containing command/read/index, edge histories, epoch/catch-up/subsecond phase, and live plus buffered calendars; valid exact cuts preserve full implemented field widths, including software-written and normalization-transient non-BCD values, while reserved bits/index/phase fail closed. Both EEPROM controllers export fixed 128-bit native images through a four-word semantic-big-endian payload adapter with stable FSM codes, model/footer size binding, reachable cross-field validation, canonical `ssLoaded=0`/`RAMWrEn=0`, exact outgoing write history, pending-write commit before ack, hidden legacy-state normalization, no synthetic register commands, and one-cycle RAM-read settling. A QSF-absent atomic-owner slice now jointly waits for instruction-boundary pause, complete device freeze, SDRAM drain, and staging acquisition; locks image generation; starts only after all barriers; releases in reverse order; and retains a fatal restore hold when safety cannot be proved. A separate QSF-absent EEPROM walker covers exact internal mono/Color and cartridge backing layouts, fixed zero padding, full prevalidation before restore writes, drain, and sticky poison ownership. The 70,193-check device ABI gate, 402,951-check walker, owner terminal-race suite, and all existing behavior tests are in `make regression`; local Yosys smoke synthesis is opt-in with `SWAN_REQUIRE_YOSYS=1`. Neither slice is instantiated in production, the remaining state data plane/exporters/CDC are absent, and `savestate_supported=0` plus `sleep_supported=false` remain unchanged |
...
| Simulation CI | Immutable simulation and trusted Quartus lab exercised; accepted workflow candidate open | checkout is pinned by full SHA; official Verilator 5.050/GCC 13.3.0 and GHDL 6.0.0 images are digest-pinned and version-checked before `make regression`; the Ubuntu runner still supplies platform-managed shell/Make/Python. The Linux/x86_64 self-hosted lane fails before checkout below 8 CPUs/30 GiB detected RAM/80 GiB free on both `RUNNER_TEMP` and the pinned Quartus image's container layer, or without a local Unix-socket Linux/x86_64 Docker daemon. The trusted DigitalOcean lab and immutable Quartus image have now completed an exact-source synthesis/fit/assembly/STA run whose strict four-corner signoff passed and whose RBF was retained. The surrounding command returned 1 only when the source-bound connectivity policy rejected its stale source binding; this is engineering evidence, not an accepted candidate or completed self-hosted release workflow. The release lane still must run the full regression and exact final public commit with a reviewed current policy, preserve host-only provenance, upload the bounded evidence set, and repeat the build for identical-RBF proof |
...
| APF package assembly | Host release gate implemented; identity approved, distribution and final evidence unavailable | `package_validator.py` fail-closes the exact release tree, every shipped APF/platform JSON, official limits and cross-file identities. Regionally Famous's original **Swan Wake** platform image is deterministically rendered at 521x165 from integer primitives and the same reviewable 18x18 swan grid as the core icon; independent generator, digest, validator, and package tests bind the source, exact binary, and ZIP payload ([`PLATFORM_ART.md`](PLATFORM_ART.md)). The 36x36 core-author icon uses Analogue's recommended 2x2 scale and documented rotated 16-bit monochrome format, with its own upright-decode/digest/staleness test. `package_core.py` reverses the supplied RBF, materializes the pinned 411-byte Chip32 loader rebuilt from this fork's extended source with the official assembler, and emits byte-identical ZIP/provenance pairs that hash every payload; release-policy V2 records public `agg23.WonderSwan` 1.0.0/1.0.1 only as predecessor history, approves the independent `RegionallyFamous.SwanSong` identity and repository, leaves Swan Song's published-release list empty, and keeps distribution-and-licensing authorization false; release mode requires `SWAN_SONG_RELEASE_EVIDENCE_V2`, which binds and recomputes the candidate-only Quartus audit from the complete artifact set, then separately requires the reviewed compression and physical Pocket/Dock gates; candidate audit success alone remains `release_eligible: false`; no final-commit V2 record, reproducibility comparison, or Pocket/Dock QA evidence has been accepted |
...
| Reproducible build identity | Generator verified; full RBF proof open | the APF pre-flow MIF uses a clean source commit, its UTC commit timestamp or explicit `SOURCE_DATE_EPOCH`, and a commit-derived 32-bit identity; focused tests prove identical MIF output across repeats/timezones plus fail-closed dirty, mismatched, malformed, and non-Git inputs; no pair of clean Quartus builds of the same final commit and epoch has produced and compared RBF hashes |
| Quartus bitstream | Exact-source engineering RBF proven; policy acceptance and reproducibility open | native macOS remains unsupported; the pinned Quartus Lite 21.1.1 Linux/amd64 container completed synthesis, fit, assembly, STA, and valid RBF generation on the trusted DigitalOcean lab from temporary non-public source commit `1e32ff6a01ea37bbe290ec7d32ec981652ad9c03` at epoch `1784047383`. The fit uses 11,761/18,480 ALMs (64%), 13,827 registers, 2,622,848/3,153,920 memory bits (83%), 289/308 RAM blocks, 23/66 DSP blocks, and 1/4 PLLs; retained `ap_core.rbf` SHA-256 is `fde228ce80fb43e6155cd64dd51022216121cd3f4ae8ac9623da78d20655a580`. The surrounding build command returned 1 solely because the source-bound connectivity policy still binds older source; compilation and strict signoff passed, but the fail-closed policy rejection means this is not an accepted candidate, a final public commit, or reproducibility proof |
| Timing closure | Exact-source four-corner signoff passes; policy acceptance and Pocket proof open | the fresh full fit reports worst setup/hold slack of `+0.477/+0.340 ns` at slow 85 C, `+0.220/+0.331 ns` at slow 0 C, `+1.745/+0.080 ns` at fast 85 C, and `+1.757/+0.010 ns` at fast 0 C. Recovery, removal, and minimum-pulse-width results are positive at every corner; eight `check_timing` diagnostics have zero findings and unconstrained-path counts are zero. Every `SWAN_SONG_SDRAM_DQ_V1` marker records exactly 16 positive setup and 16 positive hold paths: worst DQ setup/hold is `+1.115/+3.139 ns`, `+1.208/+3.056 ns`, `+4.308/+0.592 ns`, and `+4.521/+0.409 ns` in the same corner order. Warning 15069 and the unnumbered PLL RST-port warning are absent. This proves static timing only for the temporary exact-source snapshot; the stale connectivity binding, final public-commit build, second identical RBF, and physical Pocket/Dock validation remain release gates |
...
Phase 0 is not complete as a hardware deliverable until the exact final commit
passes the reproducible Quartus/evidence gates and on-device Pocket testing is
performed.

## Phase 1 — debug instrumentation

This section records Phase 1 groundwork while Phase 0's source-current
connectivity-policy, identical-RBF, release-evidence, and authorized Pocket
gates remain open. It does not declare Phase 0 deferred or Phase 1 accepted.
...
| Display-to-writer correlation | Verified on open regression | complete-from-reset manifest plus byte-lane IRAM scoreboard matches all 80,452 physical fetches: 80,010 exact CPU writers, 442 defined power-up reads, zero mismatches/collisions; `correlate_provenance.py` |
| Atomic background-cell correlation | Verified on open regression | `correlate_bg_cells.py` independently groups Screen 1/2 map/tile reads, validates 2bpp selected-word and 4bpp two-word rows, and snapshots byte writers/GDMA sources at the raw-fetch edge; regression validates 26,226 bootstrap cells, 5,177 extended-range Color cells, 8,308 Shift-JIS fixture cells, and 8,494 cells in each generated 4bpp format, with no atomic mismatch/collision |
