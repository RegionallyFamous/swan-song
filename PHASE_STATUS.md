...
| Simulation CI | Immutable simulation plus trusted Quartus-VM workflow contracts; remote runs open | checkout is pinned by full SHA; official Verilator 5.050/GCC 13.3.0 and GHDL 6.0.0 images are digest-pinned and version-checked before `make regression`; the Ubuntu runner still supplies platform-managed shell/Make/Python. A separate manual, default-branch-only Linux/x86_64 self-hosted lane fails before checkout below 8 CPUs/30 GiB detected RAM/80 GiB free on both `RUNNER_TEMP` and the pinned Quartus image's container layer, or without a local Unix-socket Linux/x86_64 Docker daemon; the probes attest guest/daemon reports, not the physical CPU or absence of emulation. The lane verifies the exact checkout and embedded build helpers, runs the complete regression before Quartus, resolves and runs the private image by immutable ID and forced entrypoints, keeps host provenance outside the writable fit mount, records privacy-stripped registry manifest digests plus a bound package manifest, restores artifact ownership, and uploads only an allowlisted bounded evidence set; one dispatch performs one fit, and the trusted runner has not yet executed it, so no remote fit is claimed |
...
| Quartus bitstream | Mac container and trusted ephemeral-VM lanes provisioned; vendor install/build unrun | native macOS remains unsupported; the pinned Linux/amd64 Docker package layer and preflight pass on Apple Silicon, while the manual x86_64 Linux VM workflow is ready for an isolated 8-vCPU/32-GB-class runner with at least 100 GB provisioned storage. Both still require the user-supplied official Quartus Lite 21.1.1.850 Linux archive with its pinned digest; installer/device probe, fit/RBF, flow/STA reports, two-clean-fit hash comparison, and evidence acceptance remain open |
