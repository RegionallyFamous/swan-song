...
| Memories v2 RTC/EEPROM device boundaries | Exact local freeze/export/load behavior and executable ABI verified; global owner/backing capture open | the RTC exports a fixed 32-byte semantic-big-endian image containing command/read/index, edge histories, epoch/catch-up/subsecond phase, and live plus buffered calendars; valid exact cuts preserve full implemented field widths, including software-written and normalization-transient non-BCD values, while reserved bits/index/phase fail closed. Both EEPROM controllers export fixed 128-bit native images through a four-word semantic-big-endian payload adapter with stable FSM codes, model/footer size binding, reachable cross-field validation, canonical `ssLoaded=0`/`RAMWrEn=0`, exact outgoing write history, pending-write commit before ack, hidden legacy-state normalization, no synthetic register commands, and one-cycle RAM-read settling. Focused GHDL behavior+synthesis plus a 70,193-check Verilator ABI gate pass and are in `make regression`; production instances remain tied off, backing-RAM walkers and the atomic all-device owner do not exist yet, and support flags remain false |
