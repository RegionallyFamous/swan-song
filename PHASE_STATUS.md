...
| Pocket launch and package truth | Definition and host-side contracts aligned; physical UX pending | the current Chip32 program queries and loads both BIOS files on every launch, so slots 9/10 are now `required: true`, read-only, and persistent-browser assets with APF `size_exact` 4,096/8,192 plus the independent RTL guards; the advertised openFPGA boundary is `.ws`/`.wsc` SD-card assets with user-supplied BIOS, not Pocket's physical-cartridge/Library path; cartridge power remains off and link/cartridge pins are unused; the system menu exposes only Auto/WonderSwan/WonderSwan Color because PocketChallenge v2 pinstrap/keypad/EEPROM/`.pc2` behavior is not implemented; `Display Orientation` and `Landscape 180°` are presentation controls while the live console orientation remains the input-matrix authority; package and first-class contract tests bind these claims, but Pocket prompts, menus, presentation, and Dock behavior still need hardware validation |
| Pocket presentation and buffering | Focused source/simulation verified; Quartus/Pocket pending | full 32-bit PAD typing rejects non-gamepad/stale packets; ten settings bits cross atomically with JSON-matching defaults; scaler slots 0/270/180 apply only at frame boundaries; related-clock RGB/control is half-cycle staged; five-bank ownership keeps up to three completed histories immutable from the faster producer, discards only superseded pending frames, and invalidates history across titles; exact rounded two/three-frame RGB444 blending preserves white; physical latency, resource fit, timing closure, Pocket/Dock output, and display modes remain mandatory gates |
