...
| Simulation CI | Immutable regression and accepted Quartus candidate workflows pass | checkout is pinned by full SHA; official Verilator 5.050/GCC 13.3.0 and GHDL 6.0.0 images are digest-pinned and version-checked before `make regression`; the Ubuntu runner still supplies platform-managed shell/Make/Python. Protected-main regression run `29377588945` passed in 19m06s. The trusted DigitalOcean lab and immutable Quartus image completed exact protected-main synthesis/fit/assembly/STA in run `29378537385`; the bounded artifact SHA-256 is `a734cd5d…57b1`, its candidate audit passes, and a separate clean build produces the same RBF/build ID. Direct local `quartus_docker.sh build` now identifies itself as development output and emits no candidate audit; only the checked-in workflow's candidate build class carries the exact GitHub run metadata and signed audit. That candidate lane still must run twice for the exact final public commit after licensing, policy, version, and release metadata changes, then pass both distinct signed bundles to the stable assembler |
...
| APF package assembly | Deterministic historical candidate produced; public release correctly blocked | `package_validator.py` fail-closes the exact release tree and definitions. `package_core.py` materializes the bit-reversed stream and pinned Chip32 loader and emits deterministic ZIP/provenance pairs. Release policy still keeps distribution-and-licensing authorization false. The stable assembler requires two distinct signed workflow executions with byte-identical RBF/build-ID outputs, then re-verifies and hash-binds the exact 31-case hardware-QA manifest and private inventory, including the tested raw RBF, version/date, nine persistent settings, installed payloads, `open-bootstrap-v3` identity, and footer-derived Open IPL variants. Packaging and staging independently reconstruct the catalogue, so bare booleans or post-QA drift cannot authorize release. |
...
| Reproducible build identity | Protected-main RBF/build ID reproduced byte-for-byte | the APF pre-flow MIF uses a clean source commit, its UTC commit timestamp or explicit `SOURCE_DATE_EPOCH`, and a commit-derived 32-bit identity; focused tests prove identical MIF output across repeats/timezones plus fail-closed dirty, mismatched, malformed, and non-Git inputs. The canonical workflow and a separate clean DigitalOcean build of `f0345ee4bae92cf137c600dfca876494cb17a5fe` produce identical RBF SHA-256 `89cd72d7…09ee5` and build-ID SHA-256 `dd62385c…0b43`; final release closure requires fresh byte-identical outputs from two distinct signed workflow executions for the exact public commit and epoch, with different job nonces |
| Quartus bitstream | Protected-main candidate accepted and byte-identically reproduced | native macOS remains unsupported; the pinned Quartus Lite 21.1.1 Linux/amd64 container completed synthesis, fit, assembly, STA, and valid RBF generation on the trusted DigitalOcean lab from protected `main` commit `f0345ee4bae92cf137c600dfca876494cb17a5fe`. The fit uses 13,207/18,480 logic elements, 16,634 registers, 2,622,848/3,153,920 memory bits, 289/308 RAM blocks, and 1/4 PLLs. The exact reviewed connectivity inventory is accepted, critical warnings are zero, and retained `ap_core.rbf` SHA-256 `89cd72d7c5e8fd829af1e1d3644aa5c6a13e188466611e61c83e1560bf409ee5` matches a separate clean build byte-for-byte. This is an accepted hardware-QA candidate, not the required pair of fresh signed final-commit executions, physical proof, or a distribution-authorized release |
| Timing closure | Accepted four-corner signoff passes; Pocket/Dock proof open | protected-main candidate `f0345ee4` reports worst setup/hold slack of `+0.426/+0.300 ns` at slow 85 C, `+0.644/+0.280 ns` at slow 0 C, `+1.741/+0.075 ns` at fast 85 C, and `+1.753/+0.030 ns` at fast 0 C. Minimum recovery, removal, and pulse-width slack is `+3.679`, `+0.264`, and `+0.753 ns`; eight `check_timing` diagnostics have zero findings and unconstrained-path counts are zero. Every `SWAN_SONG_SDRAM_DQ_V1` marker records exactly 16 positive setup and 16 positive hold paths. Critical warnings are zero and the exact connectivity inventory is accepted. This proves static timing for the hardware-QA candidate; physical Pocket/Dock validation and a rebuild after final release-facing changes remain release gates |
