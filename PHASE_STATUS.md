...
| Pocket launch and package truth | Definition and host-side contracts aligned; physical UX pending | slot 0 now uses the documented persistent-browser plus full-reload parameters, so a normal launch is expected to reuse the last title while Core Settings > Cartridge still performs a shutdown/save flush and clean title reload; Pocket's Startup Action can open openFPGA at power-on, but no documented public APF mechanism adds this core or its games to the first-party Library; firmware 2.6.0 adds a host-owned Recent category, returns quit to the openFPGA menu, and introduces Auto Dim/Auto Off power saving, but public APF exposes no supported pin/preseed hook; `core.json` declares firmware 2.3 as the minimum because it has the corrected **Reset all to defaults** browser-history behavior, while launch qualification targets 2.6.0 and now reserves a dedicated undocked Auto Off dirty-save flush/reload case; the current Chip32 program queries and loads both BIOS files on every launch, so slots 9/10 are `required: true`, read-only, and persistent-browser assets with APF `size_exact` 4,096/8,192 plus the independent RTL guards; all nine persistent Interact settings now expose exact-address readback of their requested values so PocketOS can use the documented frame read/write behavior instead of a write-only declaration; the nonpersistent **Console Setup** action recreates the original BIOS owner-registration gesture (hold Start at power-on) with a 1,048,576-cycle reset plus 33,554,432-cycle forced-Start sequencer, independent three-stage CDC levels, retrigger, and host-reset cancellation; forced Start is ORed only at logical Start and does not change Display Orientation; the advertised openFPGA boundary is `.ws`/`.wsc` SD-card assets with user-supplied BIOS, not Pocket's physical-cartridge/Library path; cartridge power remains off and link/cartridge pins are unused; the system menu exposes only Auto/WonderSwan/WonderSwan Color because PocketChallenge v2 behavior is not implemented; package and first-class contract tests bind these claims, but Recent/relaunch, prompts, title-switch save integrity, Auto Off flushing, menus, presentation, and Dock behavior still need hardware validation |
...
| APF package assembly | Deterministic historical candidate produced; public release correctly blocked | `package_validator.py` fail-closes the exact release tree, every shipped APF/platform JSON, official limits and cross-file identities. Regionally Famous's original **Swan Wake** platform image is deterministically rendered at 521x165 from integer primitives and the same reviewable 18x18 swan grid as the core icon; independent generator, digest, validator, and package tests bind the source, exact binary, and ZIP payload ([`PLATFORM_ART.md`](PLATFORM_ART.md)). The 36x36 core-author icon uses Analogue's recommended 2x2 scale and documented rotated 16-bit monochrome format, with its own upright-decode/digest/staleness test. `package_core.py` reverses the supplied RBF, materializes the pinned 411-byte Chip32 loader rebuilt from this fork's extended source with the official assembler, and emits byte-identical ZIP/provenance pairs that hash every payload. Protected-main candidate package `SwanSong-f0345ee4bae9-quartus-candidate.zip` is deterministic at SHA-256 `b458b7fd…35b4`; its sidecar records `release: false`, `build_evidence: null`, and raw RBF `89cd72d…09ee5`. Release-policy V2 still keeps distribution-and-licensing authorization false. The stable assembler requires two distinct signed workflow executions for the exact release commit and epoch, different fresh job nonces, and byte-identical RBF/build-ID outputs before it creates Release Evidence V2. It also re-verifies and hash-binds the exact 33-case hardware-QA manifest and private inventory, including the tested raw RBF, version, date, nine persistent settings, and all 13 Pocket-facing installed payloads; the new case separately binds the exact generated type-`03` save before Auto Off, after unattended Auto Off, and after relaunch. Packaging and staging independently reconstruct the complete catalogue, so bare hardware booleans or post-QA definition drift cannot authorize release. The signed records prove distinct workflow executions, not distinct physical hosts |
| Mac SD staging | Read-only by default; release installation correctly blocked by current policy | `stage_pocket_sd.py` preserves checkout-bound development staging and adds a separate exact release-verification mode bound to the caller's published ZIP SHA-256/version/source commit, strict release provenance, core identity, V2 evidence, and the checked-in release policy; current `distribution_and_licensing_authorized: false` fails before writes. BIOS files are optional user-selected exact-size inputs in release mode, game ROM contents are not read, destination identity/case/path/symlink safety preserves unrelated SD contents, mutation requires explicit `--apply`, and direct `/Volumes` targets additionally require `--allow-volume`; physical SD filesystem/eject/power-loss behavior remains outside host tests; see `POCKET_SD_STAGING.md` |
...
| Hardware equivalence | Not tested | requires user-approved Pocket/Dock validation of all 33 fixed hardware-QA cases, including unattended Auto Off dirty-save flushing |
...
| Display-to-writer correlation | Verified on open/generated regression | complete-from-reset manifests plus byte-lane IRAM scoreboards match 16,281 extended-range reads, 25,610 Shift-JIS reads, 26,168 reads in each generated planar/packed 4bpp capture, and 26,296 Screen 2 reads in each dual-window capture with zero mismatches/collisions; `correlate_provenance.py` |
| Atomic background-cell correlation | Verified on open/generated regression | `correlate_bg_cells.py` independently groups map/tile reads, validates 2bpp selected-word and 4bpp two-word rows, and snapshots byte writers/GDMA sources at the raw-fetch edge; translated regression validates 5,177 extended-range Color cells, 8,308 Shift-JIS fixture cells, 8,494 cells in each generated 4bpp format, and 8,494 Screen 2 cells in each dual-window capture; focused fixtures cover simultaneous Screen 1/2 and the window verifier also locks final composed pixels |
