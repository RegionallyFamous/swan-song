...
| Upstream pins and license audit | Provenance complete; release clearance open | `UPSTREAMS.md`; MiSTer program notice is GPL v2-or-later and is version-compatible with GPL-v3-or-later RTL; Swan Song restores Pocket's omitted program notice, packages complete GPL v2 and GPL v3 texts, and keeps the remaining combined-work and third-party reviews fail-closed in `LICENSING.md` |
| Architecture and port boundary | Complete | `ARCHITECTURE.md`, `PORTING.md` |
| System simulation | Implemented for open tests | project-authored generated REP MOVSB, dual window-boundary, Color sprite-priority, planar/packed 4bpp, type-01/type-02 32 KiB SRAM, type-03/type-04/type-05 cross-process SRAM persistence, interrupt/input, and Sound-DMA-modes probes; Wonderful's mono 80186-quirks, SoC-interrupt, internal-EEPROM, WSC extended-range, and Color Sound-DMA fixtures; the native Shift-JIS/Misaki glyph fixture; plus direct DMA/EEPROM/APF arbitration, boot/status/data-slot/RTC, full-48-bit size/result guard, startup sequencer, save-metadata CDC/table publication, A0/A4 commands, display-mode, I2S-waveform, and save-state tests via `make regression`; the persistence route runs 18 separate translated `SwanTop` processes across mono/Color and 128/256/512 KiB, requiring exact `11`→`22`→`21` status plus byte-exact save images; a pinned Wonderful `initfini` pass is recorded in `WONDERFUL_VALIDATION.md`; generated probe binaries and saves are not checked in; the 19 unlicensed inherited MiSTer test assets were removed; the direct harness does not exercise the Pocket/APF save loader, SDRAM wrapper, or physical flush path |
| APF lifecycle and slot contract | Integrated source path and focused simulation verified; compiled wrapper/Pocket open | `0082` preserves the full 48-bit byte count and independent `0` ready / `1` never / `2` later results; the guard enforces exact slot 0/11/12/13 directions and lengths, including bidirectional 128-byte mono and 2,048-byte Color console EEPROM; the game-start path uses the embedded `open-bootstrap-v3` Open IPL and has no external firmware slots. Startup remains in Setup through `008F`, delivered `0090`, metadata/table publication, and cartridge/console initialization, then requires Pocket acknowledgement of target `0140` before Idle and `0011` before Running; shutdown `0080` waits for stopped execution and the drain guard; contracts match Analogue's documented lifecycle and data-slot behavior; no physical validation is claimed |
| Memories SDRAM ownership boundary | Live ROM pass-through plus disabled stage side and global drain focused-simulated; Memories/Sleep still unsupported | channel 1 is runtime-idle after cartridge setup and now passes through an exclusive owner that latches requests until completion, gives held cartridge work acquisition priority, changes owner only after drain, routes ready/data to the latched owner, preserves a held ROM request across release, and fails closed on illegal staging access; the SDRAM controller now clears all queued/delayed transaction state on PLL init, exports global quiescence across request, command, CAS, ready, cooldown, refresh, and startup phases, and captures the complete channel-3 direction/address/data/byte-enable payload on the request edge so refresh or higher-priority arbitration cannot mutate a queued ROM/save transaction; focused tests poison every live channel-3 input after both delayed read and write pulses and still require the original payload exactly once; the wrapper retains named channel-2/channel-3 requests plus channel-3 ready; staging acquisition/request inputs remain tied low until cooperative instruction-boundary pause, real channel-3 state-engine acknowledgement, serialized transfer, lossless CDC, and exact payload padding exist; `savestate_supported=0` and `sleep_supported=false` are unchanged |
| Memories v2 preflight and RTC/EEPROM device boundaries | Full-image integrity/profile/device preflight and exact native device buses are isolated; subsystem semantic gates remain open | the RTC exports a fixed 32-byte semantic-big-endian image containing command/read/index, edge histories, epoch/catch-up/subsecond phase, and live plus buffered calendars; valid exact cuts preserve full implemented field widths, including software-written and normalization-transient non-BCD values, while reserved bits/index/phase fail closed. Both EEPROM controllers export fixed 128-bit native images through a four-word semantic-big-endian payload adapter with stable FSM codes, model/footer size binding, reachable cross-field validation, canonical `ssLoaded=0`/`RAMWrEn=0`, exact outgoing write history, pending-write commit before ack, hidden legacy-state normalization, no synthetic register commands, and one-cycle RAM-read settling. The exact RTC 256-bit and internal/cartridge EEPROM 128-bit freeze/frozen/load/image buses now pass without reordering through `memorymux` -> `SwanTop` -> `wonderswan` to `core_top`; only `core_top` ties every production freeze/load request low and every load image to zero. A QSF-absent preflight scans the exact committed `0x120100` image, checks both CRC64s, current identity/hard settings, active sizes, profile zero padding, RTC/EEPROM schemas, bounded backend completion, continuous compatibility, flash capability, and authoritative stage-content invalidation before publishing an isolated generation. It intentionally does not semantically validate CPU, PPU, APU, mapper, scheduler, DMA, or live-I/O bytes; those gates remain required before live restore. A QSF-absent atomic-owner slice jointly waits for instruction-boundary pause, complete device freeze, SDRAM drain, and staging acquisition; separate QSF-absent EEPROM walker and settle guard slices cover exact backing layouts, two-pass validation, and post-load raw-ack retention without forging acknowledgement. These isolated blocks are not instantiated: production still lacks their load-command CDC, complete subsystem exporters/validators, and state data plane. Focused full-image mutation/race/timeouts, device ABI, walker, owner, and settle-guard tests are in `make regression`. `memories_pause_request=0`, `savestate_supported=0`, and `sleep_supported=false` remain unchanged |
| Pocket launch and package truth | Definition and host-side contracts aligned; physical UX pending | Slot 0 uses the documented persistent browser and full-reload parameters, so a normal launch can reuse the last title while Core Settings > Cartridge performs a shutdown/save flush and clean reload. The Chip32 program loads only the selected `.ws`/`.wsc`; `open-bootstrap-v3` is generated from reviewed source and embedded for mono and Color startup, with bus width and owner-area policy selected from the footer snapshot. Compact images are footer/checksum validated; conventional power-of-two images retain the inherited direct loader and use their footer fields without an APF checksum gate. Slots 9/10 and the external firmware prompts are removed. All nine persistent Interact settings expose exact-address readback, and Reset core is the only momentary action. The boundary remains openFPGA SD-card assets rather than Pocket's physical-cartridge/Library path; cartridge power remains off, link/cartridge pins are unused, and PocketChallenge v2 is excluded. Recent/relaunch, title-switch save integrity, Open IPL compatibility, Auto Off flushing, menus, presentation, and Dock behavior still require physical validation. |
| Per-asset APF presets | Deterministic host tooling verified; Pocket lookup/Controls behavior pending | `pocket_per_game_preset.py` implements Analogue's documented slot-0 mirrors under `/Presets/RegionallyFamous.SwanSong/Interact/...` and `/Presets/RegionallyFamous.SwanSong/Input/...`, clones the complete live menu/control definitions, and changes only selected defaults; adversarial tests prove exact nested paths, deterministic bytes, no ROM reads or embedded ROM metadata, all-or-nothing conflict preflight, source-contract drift detection, and traversal/core-ID/symlink rejection; user-local presets remain deliberately excluded from the release core ZIP; current developer pages call Controls read-only and say remapping is coming soon, while firmware 2.4 says beta Dock remapping applies to four controllers, so the generator promises only the documented per-title definition/label/default surface and never fabricates remap state; firmware 2.6.0 Pocket/Dock must still record actual editability, application, reset, relaunch/persistence and per-title scope plus settings precedence, nested/case-varied paths, and updater preservation; see `PER_GAME_PRESETS.md`, Analogue's [`input.json`](https://www.analogue.co/developer/docs/core-definition-files/input-json), and [firmware 2.4](https://www.analogue.co/support/pocket/firmware/2.4) |
| Pocket presentation and buffering | Focused simulation and exact-commit fit complete; Pocket/Dock pending | full 32-bit PAD typing rejects non-gamepad/stale packets; thirteen settings bits, including independent Control Layout, cross atomically with JSON-matching defaults; scaler slots 0/270/180 apply only at frame boundaries; buffered frame orientation is captured when the producer completes each bank and direct-mode orientation is latched only at the output-frame boundary, preventing live orientation changes from rotating queued or mid-frame pixels; the related-clock RGB/control bundle is half-cycle staged; a two-frame APF bus bench proves exact 224x144 active output, 258 lines, single-cycle HS/VS, one scaler command per active line, reserved-word zeroing, frame-atomic grayscale, and standard 59.985 Hz cadence; the raster bench proves complete 397x258 and optional 391x258 cadence at 6.144 MHz with a frame-atomic transition; framebank ownership keeps completed histories immutable and its split 10+2-bit memories target 200 rather than a naïve 240 M10Ks; frame-atomic raw/ares color selection, rounded two-frame blending, finite 50/25/25 persistence, and newest-sample mode 3 are exact in focused arithmetic simulation. Two distinct signed workflow runs for protected-main commit `a897ecbf` reproduce the RBF/build ID byte-for-byte, fit at 277/308 M10Ks, and pass the strict four-corner audit. Physical latency, Pocket/Dock output, display-mode checks, and a rebuild after any release-facing change remain mandatory gates. |
| Phase 4 frame delivery | Production mode implemented; exact-commit fit complete and physical acceptance open | standard mode corrects the inherited APF raster from 59.386418 to 59.984769 Hz. The optional, non-default `Complete Frames 60.9Hz` mode uses a 391x258 raster at 60.905252 Hz, forces newest-complete-frame buffering, reduces modeled forced drops from 15.486929/s to 14.566446/s (5.94%), and spaces skips every 4/5 rather than 3/4 output frames. Focused RTL proves both exact rasters and frame-boundary selection; phase metrics cover every 36-cycle reset/event residue without claiming lower latency. Commit `a897ecbf` passes the strict signed Quartus fit/TimeQuest audit in two byte-identical workflow candidates. Because this mode is only 0.094748 Hz below APF's approximate 61 Hz ceiling, PocketOS Statistics=`ok`, rotation/display-mode/transition/soak captures, and Dock HDMI testing are still required before calling the improvement observable. The unsafe beam-race candidate remains explicitly outside production; see `FRAME_DELIVERY.md` and `BEAM_RACE_ANALYSIS.md`. |
| Phase 3 screen authenticity | Research-backed options implemented; real-panel calibration open | `SCREEN_AUTHENTICITY.md` follows the brief's source order: current/MiSTer RTL, official Mednafen 1.32.1 source, pinned ares, WSdev, and official APF display-mode docs. Raw RGB444 remains the neutral default; an optional exact ares Color/SwanCrystal matrix and a project-designed three-history approximation of its recursive interframe response are menu-selectable and frame-atomic. The arithmetic bench covers all 4,096 native colors, all 4,096 scalar history triples in every mode, endpoints, primaries, newest-only mode 3, and constant invariants. No WSC/CSTN or SwanCrystal/TFT response curve was found, so no arbitrary strength or hardware-accuracy claim was added. | Capture controlled color and motion charts from unmodified WSC and SwanCrystal panels, publish lighting/camera/timing methodology and uncertainty, fit separate response parameters only if the data supports them, then verify Pocket/Dock output and latency. |
...
| Simulation CI | Immutable regression and signed Quartus candidate workflows pass | checkout is pinned by full SHA; official Verilator 5.050/GCC 13.3.0 and GHDL 6.0.0 images are digest-pinned and version-checked before `make regression`; the Ubuntu runner still supplies platform-managed shell/Make/Python. Signed Quartus runs `29503168418` and `29505865127`, with different fresh nonces, both complete synthesis/fit/assembly/four-corner STA and pass the candidate audit for exact protected-main commit `a897ecbf2838fe6997628406b23de2020c04772e`; their RBF and build-ID MIF are byte-identical. Direct local `quartus_docker.sh build` identifies itself as development output and emits no candidate audit; only the checked-in workflow's candidate build class carries exact GitHub run metadata and signed audit. The complete pinned regression and candidate lane must still pass for the exact public commit after licensing, policy, version, documentation, or release metadata changes, then pass both distinct signed bundles to the stable assembler |
...
| APF package assembly | Deterministic development candidate pair exists; public release correctly blocked | `package_validator.py` fail-closes the exact release tree and definitions. `package_core.py` materializes the bit-reversed stream and pinned Chip32 loader and emits deterministic ZIP/provenance pairs. Release policy still keeps distribution-and-licensing authorization false. Signed workflow runs `29503168418` and `29505865127` provide the required distinct, byte-identical candidate shape for development commit `a897ecbf`; they do not satisfy a later release commit. The stable assembler re-verifies and hash-binds the exact 31-case hardware-QA manifest and private inventory, including the tested raw RBF, version/date, nine persistent settings, installed payloads, `open-bootstrap-v3` identity, and footer-derived Open IPL variants. Packaging and staging independently reconstruct the catalogue, so bare booleans or post-QA drift cannot authorize release. |
| Mac SD staging | Read-only by default; release installation correctly blocked by current policy | `stage_pocket_sd.py` preserves checkout-bound development staging and a separate exact release-verification mode. The package contains the built-in Open IPL and staging accepts no external firmware input; optional game ROM contents are not read. Destination identity/case/path/symlink safety preserves unrelated SD contents, mutation requires explicit `--apply`, and direct `/Volumes` targets additionally require `--allow-volume`; physical SD filesystem/eject/power-loss behavior remains outside host tests; see `POCKET_SD_STAGING.md` |
| Reproducible build identity | Protected-main RBF/build ID reproduced byte-for-byte in two signed runs | the APF pre-flow MIF uses a clean source commit, its UTC commit timestamp or explicit `SOURCE_DATE_EPOCH`, and a commit-derived 32-bit identity; focused tests prove identical MIF output across repeats/timezones plus fail-closed dirty, mismatched, malformed, and non-Git inputs. Distinct signed workflow runs `29503168418` and `29505865127` build exact commit `a897ecbf2838fe6997628406b23de2020c04772e` with different job nonces and produce identical RBF SHA-256 `951e114b1ed2e70ed11628bcfac9bc6138922cbfe45250ab56981fcdf971c03d` and build-ID MIF SHA-256 `8d1c9fb08f2a1ff95de1b326b7ef6b66952f8dc2986c2aec9c0aec32cc544a17`; final release closure requires fresh byte-identical outputs if the exact public commit or epoch changes |
| Quartus bitstream | Protected-main signed candidate pair accepted and byte-identical | native macOS remains unsupported; the pinned Quartus Lite 21.1.1 Linux/amd64 container completed synthesis, fit, assembly, STA, and valid RBF generation on the trusted DigitalOcean lab from protected `main` commit `a897ecbf2838fe6997628406b23de2020c04772e`. Both signed audits pass. The fit uses 12,974/18,480 ALMs, 16,174 registers, 2,524,544/3,153,920 memory bits, 277/308 M10K blocks, 23/66 DSP blocks, and 1/4 PLLs. The exact reviewed connectivity inventory is accepted, critical warnings are zero, and both 2,037,952-byte `ap_core.rbf` files have SHA-256 `951e114b1ed2e70ed11628bcfac9bc6138922cbfe45250ab56981fcdf971c03d`. This is an accepted development hardware-QA pair, not physical proof or a distribution-authorized release; both audits correctly record `release_eligible: false` |
| Timing closure | Signed four-corner signoff pair passes; Pocket/Dock proof open | protected-main commit `a897ecbf` reports worst setup/hold slack of `+0.255/+0.308 ns` at slow 85 C, `+0.338/+0.287 ns` at slow 0 C, `+1.745/+0.140 ns` at fast 85 C, and `+1.757/+0.083 ns` at fast 0 C. Minimum recovery, removal, and pulse-width slack across the four corners is `+5.037`, `+0.272`, and `+0.753 ns`; all total negative slack values are zero, eight `check_timing` diagnostics have zero findings, and unconstrained-path counts are zero. Every corner records exactly 16 positive SDRAM-DQ setup and 16 positive hold paths. Critical warnings are zero and the exact connectivity inventory is accepted in both signed runs. This proves static timing for that exact development commit; physical Pocket/Dock validation and a rebuild after final release-facing changes remain release gates |
| Hardware equivalence | Not tested | requires user-approved Pocket/Dock validation of all 31 fixed hardware-QA cases, including unattended Auto Off dirty-save flushing |
...
Phase 0 now has a reproducible protected-main hardware-QA candidate, but is not
complete as a public hardware deliverable until on-device Pocket/Dock testing,
distribution clearance, final release metadata, and two distinct signed
workflow executions of that exact public commit have produced byte-identical
RBF and build-ID outputs. Those signatures distinguish workflow runs, not
physical build hosts.

## Phase 1 — debug instrumentation

This section records Phase 1 groundwork while Phase 0's physical Pocket/Dock,
licensing, release-evidence, and final-public-commit rebuild gates remain open.
It does not declare Phase 0 deferred or Phase 1 accepted.
...
| VRAM/character fetch addresses | Raw and atomic runtime paths verified | focused fixtures cover all six Screen 1/2 map/tile and sprite table/tile roles with aligned completion data and collision status; translated Shift-JIS, extended-range, generated 4bpp, and Color-priority runs bind the roles each exercises to exact writers/sources and final pixels; the dual generated window probe separately locks live Screen 2 and sprite-window edges; title-specific glyph provenance remains open |
...
| Memory provenance | All eight trace-space labels runtime verified with generated open probes | paired complete-from-reset traces require 37,035 memory plus five bank events each and exact `iram`, `cart_sram`/`absent_sram`, `cart_rom0`, `cart_rom1`, `cart_rom_linear`, `boot_rom`, and mono `unmapped` behavior; checks bind ROM and generated Open IPL identities, values, masks, offsets, CPU origins, ROM aliases, and the separate GDMA chains |
...
| Pocket nonvolatile sizing and RTC boundary | Integrated source contract and focused RTL/CDC verified; compiled wrapper/Pocket flush open | cartridge slot 11 is core-specific, slot-0-cloned, restart-on-reload (`0x86`), dynamic, and capped at 524,300 bytes; the ROM-aware migration helper validates footers/checksums and only canonical or recognized inherited layouts before atomically copying a legacy shared `.sav` without source mutation or overwrite; SRAM types `01/02`/`03`/`04`/`05` use 32/128/256/512 KiB and cartridge EEPROM `10`/`20`/`50` uses 128/2,048/1,024 bytes, with a conditional exact 12-byte RTC trailer; the direct translated harness now imports and atomically exports each exact footer-sized SRAM image, and its full-regression open-probe route requires separate-process `new`→`resumed`→`resumed again` behavior for both models and types `03/04/05`; fixed core-specific console slots 12/13 independently publish table indices 2/3 as exact 128/2,048-byte `mono.eeprom`/`color.eeprom` images after `008F`; the guard returns `2` until dynamic metadata is ready, accepts only exact console-slot writes and shutdown reads, and rejects malformed lengths/directions with `1`; title changes use slot 0's full restart, which APF documents as unloading all nonvolatile slots first, while ordinary reset does not re-arm factory clearing; exact-capacity initialization, loaded-save preservation, offline conversion, and padded-cartridge-EEPROM canonical writeback are focused-tested, and a wrapper/model bench now drives factory seed, persisted-slot overlay, both clocks of the real dual-bank EEPROM backing, exact 128/2,048-byte unload comparison, model selection, and ordinary-reset retention; static backing generates keep external cartridge EEPROM bankless at 1,024×16 while only internal console EEPROM uses 2,048×16, with both elaboration paths focused-tested; compiled full-wrapper unload and physical Pocket quit/relaunch/model-switch/title-switch/manual-power-off/Auto-Off flushing remain open, and Memories/Sleep remain disabled |
| CPU ROM-to-IRAM provenance | Strictly verified with a generated clean-room REP MOVSB ROM | the versioned marker and complete v5 manifest bind two independently initialized 2,048-byte chains from mapped offsets `0x12000`/`0x15000` to IRAM `0x0800..0x0fff`/`0x2800..0x2fff`; the verifier requires trace-observed `F3 A4` origins, uninterrupted exact read/write pairs, ordered distinct instruction IDs, addresses, mapped offsets, lanes, all 4,096 values, intact final windows, completion word, terminal PC, and no extra chains; the live run records 4,168 CPU and 8,345 memory events |
...
| Boot-ROM overlay and lockout | Runtime verified for all eight Open IPL variants | the immutable generated 4 KiB/8 KiB Open IPL images enter through their exact top-of-memory reset vectors, execute their contiguous startup tails, write the footer-derived A0 hardware flags, hand off through IRAM, lock out boot memory, and require physical addresses `0xffff0..0xffffe` to change from mono boot offsets `0xff0..0xffe` or Color offsets `0x1ff0..0x1ffe` to cartridge offsets `0x1fff0..0x1fffe`; the live matrix covers mono/Color, 8/16-bit bus, and protected/writable owner-area policy with no proprietary firmware input |
| Display-to-writer correlation | Verified on open/generated regression | complete-from-reset manifests plus byte-lane IRAM scoreboards match 16,190 extended-range reads, 25,612 Shift-JIS reads, 26,170 reads in each generated planar/packed 4bpp capture, and 26,205 Screen 2 reads in each dual-window capture with zero mismatches/collisions; `correlate_provenance.py` |
| Atomic background-cell correlation | Verified on open/generated regression | `correlate_bg_cells.py` independently groups map/tile reads, validates 2bpp selected-word and 4bpp two-word rows, and snapshots byte writers/GDMA sources at the raw-fetch edge; translated regression validates 5,146 extended-range Color cells, 8,308 Shift-JIS fixture cells, 8,494 cells in each generated 4bpp format, and 8,463 Screen 2 cells in each dual-window capture; focused fixtures cover simultaneous Screen 1/2 and the window verifier also locks final composed pixels |
...
| Wonderful medium-SRAM homebrew runtime | Verified in Verilator; Pocket hardware open | a current open `wswan/medium-sram` Color build binds a pinned local CRT's physical-Color guard and `$60.7` enable before SP=`8000h` or any stack access, DS=`1000h` `.bss` clear and `.data` copy, ordered `5aa5/0000 -> a55a/c33c` cartridge-SRAM accesses, far jump into `main`, far ROM strings, exact `MEDIUM-SRAM OK` map cells, terminal HLT, and final raster; stock Wonderful 0.2.0 clears `$60.7` before its first high-stack push, while a tested SP=`4000h` override collides with the console/font workspace, so neither is accepted as-is; CRT-managed SRAM sections also must not overlap persistent records; `HOMEBREW_WONDERWITCH.md` |
| Glyph candidate report | Verified on open Japanese fixture | `report_glyphs.py` emits a deterministic, title-agnostic epoch CSV and labeled contact PNG across 2bpp/4bpp planar/packed formats and both flips; regression preserves all 591 fixture epochs while the compact unique-exact view surfaces seven bitmaps and binds the six ROM-sourced glyph candidates to their exact fingerprints, map slots, writers, IRAM spans, and source offsets; it never infers character identity |
...
`wswan/medium-sram` Color fixture proves `.data`/`.bss` initialization in
cartridge SRAM, medium-model far code, far ROM data, current console libraries,
exact SRAM mutation/readback, success cells, final pixels, and terminal HLT.
It uses a pinned, zlib-derived local CRT that requires physical Color hardware
and enables `$60.7` before selecting SP=`8000h`; stock Wonderful 0.2.0 instead
clears that gate before its first push. A tested SP=`4000h` alternative avoided
unmapped accesses but collided with the console/font workspace and did not
return from initialization. This is translated-RTL evidence only; Pocket
hardware and save persistence still require validation. Full build advice and
the discovered Color-startup/persistent-region constraints are in
...
The built-in Open IPL is intentionally a game bootstrap, not an AthenaBIOS or
FreyaBIOS service implementation. WonderWitch/Athena software that invokes
those firmware services remains outside the normal no-external-firmware path.
...
The inherited sprite-table copier also began on line 142 and advanced whenever
its free arbiter slot appeared. It now schedules one 16-bit word for each of
line 144's 256 system cycles, wraps within the 512-byte table, and captures
count/base/first at the line boundary. The cadence/count behavior follows
pinned Mesen while the all-three boundary follows pinned ares (Mesen reads
base/first live). A patterned GHDL bench covers all eight fast-forward arbiter
phases, exact final-word line attribution, wrapped descriptor data, late writes,
and fail-closed reset/unsupported-restore cancellation. Existing open-ROM frame
hashes remain the separate translated-core compatibility gate.
...
The boot-overlay probe now covers every immutable Open IPL variant. The harness
evaluates the initialized low clocks before the first boot-ROM read and derives
model, cartridge-bus width, and owner-area policy from the same footer bits as
the Pocket wrapper. Each live trace must bind the exact generated image, reset
vector, contiguous startup tail, IRAM handoff, and post-lockout cartridge
sequence. The mapper probe also exposed stale CPU
