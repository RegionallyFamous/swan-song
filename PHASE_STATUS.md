...
| Memories SDRAM ownership boundary | Live ROM pass-through plus disabled stage side focused-simulated; Memories/Sleep still unsupported | channel 1 is runtime-idle after cartridge setup and now passes through an exclusive owner that latches requests until completion, gives held cartridge work acquisition priority, changes owner only after drain, routes ready/data to the latched owner, preserves a held ROM request across release, and fails closed on illegal staging access; its staging acquisition/request inputs are tied low in production until cooperative instruction-boundary pause, real channel-3/global SDRAM completion, serialized state-engine transfer, lossless CDC, and exact payload padding exist; `savestate_supported=0` and `sleep_supported=false` are unchanged |
