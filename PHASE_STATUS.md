...
| ROM input capacity | Implemented Pocket policy bounded to 16 MiB | slot 0 accepts only power-of-two images from 64 KiB through 16 MiB because this core's mapper path is 24-bit; [WSdev's ROM-header table](https://ws.nesdev.org/wiki/ROM_header) documents known encodings through 16 MiB, while its [later Bandai 2003 mapper](https://ws.nesdev.org/wiki/Mapper) exposes six 1 MiB linear-bank bits, implying a theoretical 64 MiB address capacity that is not implemented or advertised here; the [Wonderful target](https://wonderful.asie.pl/docs/target/wswan/) remains the modern open-toolchain reference, not evidence for larger runtime support |
...
| Internal EEPROM protocol | Open fixture passes 23/23 in deterministic translated RTL | pinned Wonderful mono fixture now produces byte-identical paired traces, 184 complete PASS rows, terminal PC `0xff620`, and exact target pixels; focused GHDL covers separate write/read latches, single read/write/erase, EWDS/EWEN, WRAL/ERAL, protected versus user words, invalid controls, DONE/READY, and saved-EWDS restoration; the ten-enable READ delay is a deterministic functional model and does not establish physical latency, Color-hardware mono mode, or power-cycle persistence |
| Pocket nonvolatile sizing and RTC boundary | Integrated source contract and focused RTL/CDC verified; wrapper/Pocket flush open | dynamic slot is capped at 524,300 bytes; SRAM types `01/02`/`03`/`04`/`05` use 32/128/256/512 KiB and EEPROM `10`/`20`/`50` use 128/2,048/1,024 bytes, with a conditional exact 12-byte RTC trailer; metadata crosses atomically to `clk_74a` and publishes table index 3 / ID 11 only after `008F`, remaining alive across Reset Enter for flush; the guard returns `2` until metadata is ready, then accepts absent/canonical sizes plus only the 2,060-byte legacy RTC EEPROM case and rejects malformed or legacy type-`01` lengths with `1`; shutdown readback also returns `2` until execution has stopped and its fixed 31-`clk_74a` drain guard completes; exact-capacity initialization, loaded-save preservation, offline conversion, and padded-EEPROM canonical writeback are focused-tested; compiled full-wrapper unload and physical Pocket quit/relaunch/power-off flushing remain open, and Memories/Sleep remain disabled |
...
baseline contains APF Memories/sleep-wake controller logic, RTC logic, vertical
control remapping, triple buffering, and two/three-frame flicker blending. The
