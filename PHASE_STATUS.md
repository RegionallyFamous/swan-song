...
| System simulation | Implemented for open tests | deterministic GPU-framebuffer hashes for two checked-in MiSTer tests, Wonderful's mono 80186-quirks, SoC-interrupt, internal-EEPROM, WSC extended-range, and Color Sound-DMA fixtures, the native Shift-JIS/Misaki glyph fixture, paired build-generated planar/packed 4bpp and type-01/type-02 32 KiB SRAM probes, build-generated Color sprite-priority, interrupt/input, and Sound-DMA modes probes, plus direct DMA/EEPROM/APF arbitration, boot/status/data-slot/RTC, A0/A4 commands, display-mode, I2S-waveform, and save-state tests via `make regression`; a pinned Wonderful `initfini` pass is recorded in `WONDERFUL_VALIDATION.md`; generated probe binaries are not checked in; physical SDRAM and Pocket hardware remain outside this harness |
...
| Pocket nonvolatile sizing and RTC boundary | Exact source contract and focused RTL/CDC verified; full lifecycle open | dynamic slot is capped at 524,300 bytes; all SRAM/EEPROM types publish exact canonical payload bytes with a conditional 12-byte RTC trailer; new-title state no longer retains the preceding save/RTC readiness; an acknowledged asynchronous-clock RTC bridge proves coherent event delivery; a focused RTL lifecycle bench proves exact-capacity `0xffff` initialization for absent external EEPROM, loaded-save preservation, and no re-clear on later reset/shutdown edges; offline legacy conversion and padded-EEPROM load compatibility are covered, while atomic header-to-data-table publication, fail-closed incoming length validation, full wrapper unload, and Pocket quit/sleep flush remain open |
