...
| Simulation CI | Immutable simulation plus trusted Quartus-VM workflow contracts; remote runs open | checkout is pinned by full SHA; official Verilator 5.050/GCC 13.3.0 and GHDL 6.0.0 images are digest-pinned and version-checked before `make regression`; the Ubuntu runner still supplies platform-managed shell/Make/Python. A separate manual, default-branch-only native Linux/x86_64 self-hosted lane verifies the exact checkout and embedded build helpers, runs the complete regression before Quartus, restores artifact ownership, and uploads only an allowlisted bounded evidence set; the current branch has not been pushed to a configured fork or dispatched on that trusted runner, so no remote green run or fit is claimed |
...
| Quartus bitstream | Mac container and trusted ephemeral-VM lanes provisioned; vendor install/build unrun | native macOS remains unsupported; the pinned Linux/amd64 Docker package layer and preflight pass on Apple Silicon, while the manual native-x86_64 VM workflow is ready for an isolated 8-vCPU/32-GB-class runner. Both still require the user-supplied official Quartus Lite 21.1.1.850 Linux archive with its pinned digest; installer/device probe, fit/RBF, flow/STA reports, and evidence acceptance remain open |
